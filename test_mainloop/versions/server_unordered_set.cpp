...
void cleanup(std::unordered_set<int> open_fds)
{
    for (std::unordered_set<int>::iterator it = open_fds.begin(); it != open_fds.end(); ++it)
        close(*it);
}
...
int error_msg(ErrorFlag::Type type, std::unordered_set<int> open_fds)
{
    int ret = 1;
    switch (type)
    {
