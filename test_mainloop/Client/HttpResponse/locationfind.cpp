...
unsigned int getLocation(const std::string& match, const t_server& serverConfig) {
...
void    attachPrefix(const std::string& uri, std::string& path, t_location& location) {
...
/**
    * @brief Uses the URI using config data to return the resolved path and http code.
    * @param uri          The request URI from the client.
    * @param serverConfig The server configuration associated with this client.
    * @return UriResult - httpCode to determine action, path to execute action with, bool for autoindex.
    */
