...
#define BUFFER_SIZE 7 // 4.096 standard
...
void cleanup(int open_fds[MAX_CLIENTS + 3]) {
  size_t iter = 3;
  while (iter < MAX_CLIENTS + 3) {
    if (open_fds[iter] != -1)
      close(open_fds[iter]);
    ++iter;
  }
...
int error_msg(ErrorFlag::Type type, int open_fds[MAX_CLIENTS]) {
  ...
  case ErrorFlag::ERR_LISTEN:
    ...
    break;
  case ErrorFlag::ERR_SEND:
    custom_perror("send: ", strerror(errno));
    break;
...
void set_arr(int open_fds[MAX_CLIENTS + 3]) {
  size_t iter = 0;
  while (iter < MAX_CLIENTS + 3) {
    open_fds[iter] = -1;
    ++iter;
