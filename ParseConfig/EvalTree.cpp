...
void    evalDirective(Node *tree, t_eval &evalData) {
    if (!evalData.serverFlag) {
        switch (tree->tag) {
            case MAX_CLIENTS: parseNumberGlobal(tree->args, &evalData.maxClients); return;
            case CLIENTS_PER_SERVER: parseNumberGlobal(tree->args, &evalData.clientsPerServer); return;
            default: throw std::runtime_error("wrong directive outside of server block");
        } 
    }
    t_location currLocation;
    if (!evalData.locationFlag)
        currLocation = evalData.servers.back().locations.front();
    else
        currLocation = evalData.servers.back().locations.back();
    switch (tree->tag) {
        case ROOT: parseRoot(tree->args, currLocation); return;
        case INDEX: parseIndex(tree->args, currLocation); return;
        case AUTOINDEX: parseAutoindex(tree->args, currLocation); return;
        case ALLOWMETHODS: currLocation.allowMethods = allowMethods(tree->args); return;
        default :   ;
    }
    if (evalData.locationFlag)
    ...
    switch (tree->tag) {
        ...
        case CLIENT_MAX_BODY_SIZE: parseMaxBody(tree->args, evalData.servers.back()); break ;
        case LISTEN: parseListen(tree->args, evalData.servers.back()); break;
        case LOCALHOST: parseLocalhost(tree->args, evalData.servers.back()); break;
...
void    evalContext(Node *tree, t_eval &evalData) {
    if (tree->tag == SERVER) {
...
void evalTree(Node *tree, t_eval &evalData) {
